...
export default function ZoneList({ userLocation, onZoneClick }: ZoneListProps) {
  const [zonesWithDistance, setZonesWithDistance] = useState<ZoneWithDistance[]>(zones)
  ...
  const filteredZones = zonesWithDistance.filter((zone) =>
                  ...
                  {zone.distance && (
                    <div className="flex items-center mt-2 ml-6">
                      <Circle className={`w-3 h-3 mr-2 ${
                        zone.distance < 5 ? 'fill-green-500 text-green-500' : 'fill-red-500 text-red-500'
                      }`} />
